...
const UpdatesPanel = forwardRef<{ togglePanel: () => void }, UpdatesPanelProps>(({ updates = [], onPanelOpen }, ref) => {
  ...
  return (
    <>
      ...
      {/* Updates Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-[#e71d36] shadow-2xl z-40 transition-transform duration-200 ease-out ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="px-8 pt-8 pb-6">
            ...
            <p className="text-sm text-[#ffffff]/60 mt-1 text-pretty">Latest heartbeat events</p>
          </div>
            ...
            <div className="px-8 space-y-6">
              {updates.length === 0 ? (
                <div className="text-center py-12">
                  <p className="text-[#ffffff]/60 text-sm text-pretty">No updates yet</p>
                  <p className="text-[#ffffff]/40 text-xs mt-2 text-pretty">Select an emotion to create an update</p>
                </div>
              ) : (
                updates.map((update, index) => (
                  <div key={update._id} className="group">
                    ...
                    <p className="text-[#ffffff] leading-relaxed text-pretty">
                      {update.content}
                    </p>
